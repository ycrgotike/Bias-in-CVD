...
@app.route('/female-predict', methods=['GET','POST'])
def female_predict():

    if request.method == "GET":
        return render_template("female_predict.html", results=None)
        ...
        results = predict_pipeline.femalepredict(pred_df)

        prediction = int(results[0])

        return render_template("female_predict.html", results=prediction)
